    ...
    <div className="flex flex-col h-full bg-gradient-to-br from-black to-emerald-900 text-white rounded-lg shadow-lg overflow-hidden border-none">
      ...
      {messages.length <= 1 && (
        <div className="px-4 pb-2">
          <h3 className="text-xs text-gray-400 mb-2">SUGGESTED QUESTIONS</h3>
          <div className="grid grid-cols-2 gap-2">
            {suggestedQuestions.map((question, i) => (
              <button
                key={i}
                onClick={() => setInput(question)}
                className="text-xs text-left truncate p-2 rounded-lg bg-gray-700/80 hover:bg-gray-600/80 disabled:opacity-50"
                disabled={isLoading}
              >
                {question}
              </button>
